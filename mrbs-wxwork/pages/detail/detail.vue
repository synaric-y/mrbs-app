<script setup>
	...
	const isFirst = ref(!entryId) // 是否为编辑态
	...
	const returnToIndex = () => {
		...
		gotoPage('../index/index',{
			...
			area_name: route?.query?.area_name ?? 'all'
		})
	...
	const getMeetingById = () => {
		...
		return new Promise((resolve,reject)=>{
			...
			wrappedGetMeetingById(entryId,false)
			.then((data)=>{
	...
	const reserveMeeting = () => {
		...
		wrappedReserveSingleMeeting(
			...
			.catch(error => {
				showToast(i18n.global.t('detail.reserve_notify.fail') + error, 5000);
			})
			.finally(() => {
				// returnToIndex()
			})
	...
	const currentDate = ref(route.query.time ? new Date(parseInt(route.query.time)) : new Date()) // 当前日期，双向绑定
...
<template>
	<Layout>
		...
		<template #header>
			<Header :title="$t('detail.title')+' '+(room.room_name ? room.room_name:'')" :isReturn="true" @return="returnToIndex"/>
