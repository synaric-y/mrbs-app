<script setup>
	...
	const selectionTranslationStart = (event) => {
		...
		// console.log(selectionStartX.value,selectionStartLeftHr.value,selectionStartRightHr.value)
		...
		<ScrollContainer :initPos="initPos" :totalHeight="'7rem'" :timelineHeight="'2.5rem'" :hasTag="true"
			...
			<template #timeline-bar>
				...
				<div class="period selecting-period" v-if="!disabled" :style="{
			        ...
			        }" @touchstart="selectionTranslationStart" @touchmove="selectionTranslationMove"
					@touchend="selectionTranslationEnd" @mousedown="selectionTranslationStart" />
				<div :class="'handle handle-left ' + leftHandleBlink" v-if="!disabled"
					:style="{left:positionLeft,cursor:'pointer'}" @touchstart="handleStartLeft"
					@touchmove="handleMoveLeft" @touchend="handleEndLeft" @mousedown="handleStartLeft" />
				<div :class="'handle handle-right ' + rightHandleBlink" v-if="!disabled"
					:style="{left:positionRight,cursor:'pointer'}" @touchstart="handleStartRight"
					@touchmove="handleMoveRight" @touchend="handleEndRight" @mousedown="handleStartRight" />
			</template>
