<script setup>
	...
	const gotoEdit = (status, repeat_id, id, room_id) => {
		...
		if (!repeat_id) { // 单次
			...
			repeatedEditDisplay.value = true
...
<template>
						...
						<template v-slot:right>
							<div class="btns">
								<template v-if="active!=2">
									...
									<view class="btn force-end-button" v-if="active==1" @click="confirmForceEndMeeting(entry.id)">
	...
	<uni-popup ref="refPopup" :mask-click="false">
		<RepeatedEditDialog v-if="repeatedEditDisplay" :repeat_id="currentRepeatId" :entry_id="currentEntryId" @close="refPopup.close()"
